...
if ( ! class_exists( 'WP_Bootstrap_Navwalker' ) ) :
	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_lvl( &$output, $depth = 0, $args = null ) {
			...
			$classes = array( 'multistep-menus row' );
			/**
			...
			$output .= "{$n}{$indent}<div class=\"multistep-wrap\"><div class=\"container\"><ul$class_names $labelledby>{$n}";
		}
		...
		public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
			...
			if ( $this->has_children ) {
				$classes[] = 'dropdown has-multistep';
			}
			...
			if ( $this->has_children && 0 === $depth ) {
				...
				$atts['class']         = 'multistep-toggle dropdown-toggle nav-link';
			...
			if( $depth > 0 ) {
				if( is_object( $item ) && isset( $item->ID ) ) {
			        ...
			        if ( ! empty( $menu_item_desc ) ) {
			            $item_output .= '<p>' . $menu_item_desc . '</p>';
			        }
			    }
				$item_output .= '</div>';
			}
